...
def eval_loss(gt_info, pre_info):
    ...
    GT_SUM = 0
    eval_img_num = 0
    print('imginfonum:',len(pre_info))
    pbar = tqdm.tqdm(pre_info)
    for imgname in pbar:
        if imgname in gt_info:
            ...
            for pre_single in pre_data:
                ...
                if len(gt_data) != 0:
                    ...
                    if box_iou_max > 0.5:
                        TP += 1
                        if  -1 not in gt_data[maxindex] and len(gt_data[maxindex])>5:
...
if __name__ == "__main__":
    ...
    _, gt_dataset = obt_gt_data(sys.argv[1])
    # print(gt_dataset)
    _, pre_dataset = obt_pre_data(sys.argv[2])
    # print(pre_dataset)
